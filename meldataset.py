...
def get_dataset_direct_filelist(a):
    with open(a.input_training_file, "r", encoding="utf-8") as fi:
...
class MelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._cache_ref_count == 0:
            ...
            if sampling_rate != self.sampling_rate:
                audio = ta.functional.resample(
                    audio, orig_freq=sampling_rate, new_freq=self.sampling_rate
                )
                # raise ValueError(
        ...
        spk_emb = (
            torch.tensor(self.wav_to_spk_emb[filename]) if self.wav_to_spk_emb else None
        )
        return (mel.squeeze(), audio.squeeze(0), filename, mel_loss.squeeze(), spk_emb)
